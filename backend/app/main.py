...
if my_key is None:
    print(f"WARNING: Unable to locate OpenAI API token\n"
          f"Ensure file '.env' exists with backend app directory and token is set to variable "
          f"OPENAI_API_KEY")
    exit()
...
def get_story_part(prompt: str, story_history: list) -> str:
    ...
    if story_history:
        completion = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[{"role": "system", "content": f"This list is the story history for context: {story_history}"},
...
if __name__ == "__main__":
    ...
    initial_text_prompt = (f"Give me an introduction to a {context.genre} story with a character named {user_name}. "
                           f"This introduction shall not exceed {context.max_text_length}.")
    ...
    while True:
        ...
        image_prompt = prompt_generator.get_image_prompt(context, current_paragraph)
